...
import axios from 'axios';
 ...
 export const getgames = createAsyncThunk ("games/get",async ()=> {
    try {
        let result = axios.get("http://localhost:5006/game/");
        return result; 
    } catch (error) {
        console.log(error)
...
export const addgame = createAsyncThunk ("games/add",async (game)=> {
  try {
      let result = axios.post("http://localhost:5006/game/add", game);
      return result; 
  } catch (error) {
      console.log(error)
...
export const deletegame = createAsyncThunk ("games/delete",async (id)=> {
  try {
              let result = axios.delete(`http://localhost:5006/game/${id}`);
      return result; 
  } catch (error) {
      console.log(error)
...
export const updategame = createAsyncThunk ("games/update",async ({id, game})=> {
  try {
              let result = axios.put(`http://localhost:5006/game/${id}`, game);
      return result; 
  } catch (error) {
      console.log(error)
